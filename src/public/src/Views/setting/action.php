...
if ($action === "user-create") {
  try {
    ...
    if (empty($email)) {
      $VALIDATION->Alert("danger", "ไม่พบข้อมูล กรุณาลองอีกครั้ง!", "/setting/user-create");
    }

    $count = $USER->UserCount([$email]);
    if (intval($count) > 0) {
      $VALIDATION->Alert("danger", "ข้อมูลซ้ำในระบบ กรุณาลองอีกครั้ง!", "/setting/user-create");
    }

    $USER->UserCreate([$firstname, $lastname, $email, $contact]);

    $VALIDATION->Alert("success", "ดำเนินการเรียบร้อย!", "/setting/user");
  } catch (PDOException $e) {
    die($e->getMessage());
  }
}
...
if ($action === "user-update") {
  try {
    ...
    $USER->UserUpdate([$firstname, $lastname, $email, $contact, $level, $status, $id]);
    $VALIDATION->Alert("success", "ดำเนินการเรียบร้อย!", "/setting/user-view/{$uuid}");
  } catch (PDOException $e) {
    die($e->getMessage());
  }
}
